...
var GetList = function (that) {
  ...
  wx.request({
    url: 'https://route.showapi.com/341-1', //仅为示例，并非真实的接口地址
    data: {
      showapi_appid: '59144',
  ...
  onLoad: function () {
    ...
    wx.request({
      ...
      success: function (res) {
        ...
        var data1 = res.data.showapi_res_body.contentlist;
        yeshu = res.data.showapi_res_body.allPages
        for (var i = 0; i < res.data.showapi_res_body.contentlist.length;i++){
          var dataZ1 = [];
          // 内容
          dataZ1[0] = data1[i].text.replace(/(<br\s?\/?>)+/gi, "\t");
